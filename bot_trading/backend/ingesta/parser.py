...
logger = logging.getLogger("trading_bot.parser")
...
# Filtro estricto de mensajes publicitarios, educativos o actualizaciones informativas (NO son órdenes)
...
# Patrón robusto no capturante para capturar números con coma (,) o punto (.) como separador decimal o de miles
...
def sanitize_price_str(raw_str: str) -> Optional[Decimal]:
    ...
    y asegurando el punto (.) decimal estándar requerido por Python Decimal y APIs de Brokers.
    Ejemplos soportados:
    ...
    # Si contiene punto y coma a la vez (separador de miles + decimal)
    if '.' in s and ',' in s:
        # Formato europeo: '4.383,69' (punto miles, coma decimal)
        if s.find('.') < s.find(','):
            s = s.replace('.', '').replace(',', '.')
        else:
            # Formato anglosajón: '4,383.69' (coma miles, punto decimal)
            s = s.replace(',', '')
    elif ',' in s:
        # Solo coma: '4383,69' -> convertir a punto
        s = s.replace(',', '.')
...
def parse_signal(
    ...
    # 1. Comprobar Modificadores primero (Move SL to 4501, Set BE, Close Now)
    modifier_event = _check_modifiers(cleaned_text, message_id, channel_id, reply_to_msg_id)
    if modifier_event:
        return modifier_event

    # 2. Descartar mensajes informativos / spam / marketing
    if RE_IGNORE_INFORMATIONAL.search(cleaned_text):
        return None

    # 3. Comprobar si menciona XAUUSD / GOLD
    ...
    # 4. Determinar Dirección (BUY / SELL) con prioridad a palabras clave
    side = _determine_order_side(cleaned_text)
    if not side:
        return None

    # 5. Extraer Entry Price
    entry_price = _extract_entry_price(cleaned_text)
    if not entry_price:
        return None

    # 6. Extraer Stop Loss
    sl_price, requires_dynamic_sl = _extract_sl(cleaned_text)

    # 7. Extraer Take Profits (TP1, TP2, TP3... o Pips relativos)
    tp_levels = _extract_tps(cleaned_text, entry_price, side)
    if not tp_levels:
        return None
...
def _determine_order_side(text: str) -> Optional[OrderSide]:
    """Determina la dirección priorizando palabras clave explícitas sobre emojis."""
...
def _check_modifiers(
    ...
    reply_to_msg_id: Optional[int] = None
) -> Optional[ModifierSignalEvent]:
    """Detecta modificadores de órdenes activas (Move SL, Set BE, Close)."""
    # Move SL to [price]
...
def _extract_entry_price(text: str) -> Optional[Decimal]:
    """Extrae y normaliza el precio de entrada."""
...
def _extract_sl(text: str) -> Tuple[Optional[Decimal], bool]:
    """Extrae Stop Loss explícito o marca dinámico."""
...
def _extract_tps(text: str, entry: Decimal, side: OrderSide) -> List[Decimal]:
    """Extrae lista ordenada de Take Profits o calcula TP relativo por pips."""
    tps_dict = {}

    # 1. Buscar TP1, TP2, TP3 numerados con precio absoluto (ej. TP1: 4488 o 4380,69)
    ...
    # 2. Buscar formato relativo de pips (ej. Set TP1 +30 Pips)
    ...
    # 3. Buscar bloque genérico TP: 4488, 4483, 4475
    ...
    # 4. Fallback: extraer números restantes posteriores a Entry
...
def _validate_mathematical_coherence(
    ...
    """
    BUY: Entry > SL y TP1 > Entry.
    SELL: Entry < SL y TP1 < Entry.
    """
    tp1 = tps[0]
    if side == OrderSide.BUY:
        if sl >= entry:
            return False, f"En BUY el SL ({sl}) debe ser menor que Entry ({entry})"
        if tp1 <= entry:
            return False, f"En BUY el TP1 ({tp1}) debe ser mayor que Entry ({entry})"
    elif side == OrderSide.SELL:
        if sl <= entry:
            return False, f"En SELL el SL ({sl}) debe ser mayor que Entry ({entry})"
        if tp1 >= entry:
            return False, f"En SELL el TP1 ({tp1}) debe ser menor que Entry ({entry})"
