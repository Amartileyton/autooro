  ...
  return (
    <header className="flex justify-between items-center w-full px-4 h-12 bg-surface border-b border-outline-variant shrink-0 z-10 relative">
      <div className="flex items-center gap-4">
        {/* Título en Gris Plateado Elegante */}
        <div className="text-headline-md font-bold text-slate-200 tracking-tight flex items-center gap-2.5">
          {/* CASH LOGO.svg en Gris / Plata */}
          <svg className="w-6 h-6 fill-current text-slate-300 shrink-0" viewBox="0 0 32 32">
          ...
          <span>GOLD-EX TERMINAL</span>

          {/* Indicador de Punto con Dispersión de Humo (Botón de Diagnóstico de APIs) */}
          <button
            onClick={onOpenDiagnostics}
            title={botActive ? "Bot Activo (24/7) — Clic para abrir Diagnóstico de APIs" : "Bot Pausado — Clic para abrir Diagnóstico de APIs"}
            className="relative flex items-center justify-center p-1.5 rounded-full hover:scale-125 transition-transform duration-200 cursor-pointer ml-1 focus:outline-none"
          >
            <div
        ...
        {/* Telemetría en tiempo real: BALANCE -> XAUUSD SPOT */}
        <div className="hidden lg:flex items-center gap-6 ml-2 pl-6 border-l border-outline-variant">
          <div className="flex items-center gap-2">
            <span className="text-label-sm text-slate-400 font-semibold tracking-wider">BALANCE</span>
            {balance !== null && balance !== undefined && hasLiveBalance ? (
              <span className="text-data-md font-mono font-bold text-slate-100">
                ${balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </span>
            ) : (
              <span className="text-data-sm font-mono text-slate-400/80 italic tracking-tight">
                No disponible
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <span className="text-label-sm text-slate-400 font-semibold tracking-wider">XAUUSD SPOT</span>
            <span className="text-data-md font-mono text-slate-100 pulse-live font-bold">
      ...
      <div className="flex items-center gap-3">
        <button 
          ...
          className="text-slate-300 bg-surface-container hover:bg-surface-container-high hover:text-white border border-outline-variant transition-all px-2.5 py-1 rounded flex items-center gap-1.5 font-medium shadow-sm"
        >
          <span className="material-symbols-outlined text-[18px]">settings</span>
          <span className="text-label-sm hidden sm:inline uppercase tracking-wider">Controles</span>
        </button>
      </div>
    </header>
  );
